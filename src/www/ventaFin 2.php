...
<html>
    <head>
        ...
        <?php
			# Comprobamos la existencia de parámetros GET. En caso de no estar definidos: redireccionamos a documento origen.
            if(!isset($_GET['factura']) || !isset($_GET['idCli'])){
				header('location: venta.php');
			}else{
				$FACTURA	= $_GET['factura'];
				$IDCLI		= $_GET['idCli'];
				
				if(finVenta($FACTURA,$IDCLI)){ # crea la nueva factura en tabla FACTURAS y elimina contenido de tabla VENTAS. Devuelve true/false
					header("location: venta.php?op=ok"); 
				}else{
					header("location: venta.php?op=ko");
				}
			}
        ?>
    </head>
