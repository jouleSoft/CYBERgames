...
<html>
    <head>
        ...
        <?php
            ...
            if(!isset($_POST['telefono1']) || !isset($_POST['telefono2']) || !isset($_POST['email']) || !isset($_POST['direccion']) || !isset($_POST['cpostal'])){
				print "Error en parámetro POST";
			}else{
				...
				$IDCLI		= strtoupper((string)$_POST['dni']);
				...
				if(existeCliente(strtoupper($_POST['dni']))){
					...
					if($mysqli->error_connect){
						...
						# Añadir cliente a tabla CLIENTES
						$sql = "INSERT INTO CLIENTES VALUES('$IDCLI', '$NOMBRE', '$APELLIDOS', '$TELEFONO1', '$TELEFONO2', '$EMAIL', '$DIRECCION', $CPOSTAL)";
						
						if($res = $mysqli->query($sql)){
							$url = "clientes.php?op=ok";
						}else{
							$url = "clientes.php?op=ko";
						}
