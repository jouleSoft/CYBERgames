...
<html>
    <head>
        ...
        <?php
			...
			# El parámetro 'plat' se habrá introducido a través de un evento javaScript 'prompt'.
            ...
            if(!isset($_GET['plat']) || !isset($_GET['idArt']) || !isset($_GET['op'])){
				print "Error en parámetro GET";
			}else{
				$PLAT	= strtoupper($_GET['plat']);
				$PLAT	= strtr($PLAT,"áéíóúü","ÁÉÍÓÚÜ");
				$IDART	= $_GET['idArt'];
				$OP		= $_GET['op'];
				...
				if(!$mysqli->connect_error){
					$sql = "SELECT * FROM PLATAFORMAS WHERE PLATAFORMA LIKE '$PLAT'";
					if($res=$mysqli->query($sql)){
						if($res->num_rows == 0){
							$sql = "INSERT INTO PLATAFORMAS VALUES(NULL,'$PLAT');";
							if($res=$mysqli->query($sql)){
